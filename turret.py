...
class Turret(pg.sprite.Sprite):
    def __init__(self, sprite_sheets, tile_x, tile_y):
        ...
        self.tile_x = tile_x
        self.tile_y = tile_y
        #calcualte the center coordinates
        self.x = (self.tile_x + 0.5) * c.TILE_SIZE
        self.y = (self.tile_y + 0.5) * c.TILE_SIZE

        #animation variables
        self.sprite_sheets = sprite_sheets
        self.animation_list = self.liad_images(self.sprite_sheets[self.upgrade_level - 1])
        self.frame_index = 0
    ...
    def load_images(self, sprite_sheet):
        ...
        animation_list = []
        for x in range(c.ANIMATIONS_STEPS):
    ...
    def pick_target(self, enemy_group):
        ...
        x_dist = 0
        y_dist = 0
        #check distance to each enemy to see of it's in range
    ...
    def play_animation(self):
        #update image
        self.original_image = self.animation_list[self.frame_index]
        #check if enouph time has passed since the last update
        if pg.time.get_ticks() - self.update_time > c.ANIMATION_DELAY:
            ...
            if self.frame_index >= len(self.animation_list):
                ...
                self.last_shot = pg.time.get.ticks()
                self.target = None
